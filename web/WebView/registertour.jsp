...
<!--         <meta http-equiv="refresh" content="1">-->
  ...
  String sel,sno,tid,sell,ss="",cno="",Astatus="",Sstatus="",apid="",id="",pimg="",fee="";
  ...
  ResultSet rs=con.select(sel);
    // out.println(sel);
    String sel5="select * from tbl_applytournament where  tournament_id='"+request.getParameter("regid")+"' and  player_id='"+session.getAttribute("player")+"'";
    ...
    <body align='center'>
        ...
        <i class='fas fa-user' style='font-size:30px;color:red; padding: 20px;'></i>Booked
       <i class='fas fa-user' style='font-size:30px;color:gray;padding: 20px;'></i>Available
      <i class='fas fa-user' style='font-size:30px;color:green;padding: 20px;'></i>Selected
      ...
      while(rs.next())
      {
        ...
        sell="select * from tbl_applytournament a inner join tbl_players pl on pl.player_id=a.player_id inner join tbl_slot s on a.slot_id=s.slot_id where a.tournament_id='"+tid+"' and s.slot_id='"+sno+"'"; 
         ...
         {
        %>
        <i class='fas fa-user' style='font-size:30px;color:gray;padding: 20px;' id="s<%=sno%>" onclick="insert(<%=sno%>,<%=tid%>),cl(<%=sno%>,<%=tid%>)"></i>  
          <%
          
         }
         else if(Astatus.equals("1") && Sstatus.equals("1"))
         {%>
      <i class='fas fa-user' style='font-size:30px;color:red;padding: 20px;' onclick="alert('Already Booked !!!')"></i>
      ...
      Astatus="";
         ...
         {%>
        <i class='fas fa-user' style='font-size:30px;color:green;padding: 20px;' id="s<%=sno%>" onclick="cl(<%=sno%>,<%=tid%>)"></i>
      <%
      Astatus="";
          Sstatus="";
         }
...
<%
            ...
            if(request.getParameter("zid")!=null)
             ...
             if(rs6z.next())
             {
                 ...
                 {
                      response.sendRedirect("tournamentlist.jsp?id="+session.getAttribute("player"));
                 }
             }
             }
